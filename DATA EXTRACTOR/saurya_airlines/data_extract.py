...
dp = open('saurya_airlines/tickets.json', 'r')
data = json.load(dp)['tickets']
# print(data)
total = len(data)
fp = open('saurya_airlines/names.json', 'r')
det = json.load(fp)
count = 1
error_count = 0
f = open('saurya_airlines/names.json', 'r')
d = json.load(f)
...
for i in data:
    if i['ticket_number'] not in ticket_numbers:
        try:
            ...
            errors = {}
            errors['data'] = i
            fep = open('saurya_airlines/errors.json', 'r')
            delt = json.load(fep)
            delt.append(errors)
            flep = open('saurya_airlines/errors.json', 'w+')
            json.dump(delt,flep)
    count +=1
    print(count)
    
fp = open('saurya_airlines/names.json', 'w+')
json.dump(d,fp)
