...
class GunaAir(AirlineWebsite):
    ...
    def search_pnr_by_ticket_number(self,ticketnumber):
        ...
        response = requests.post('http://flyguna.org/SaOnlineReservation/searchpage.jsp',cookies=self.cookies, data=data, verify=False)
        soup = bs4.BeautifulSoup(response.content,'lxml')
        return soup.find("table").find_all("tr")[3].find_all("td")[1].get_text(),soup.find("table").find_all("tr")[3].find_all("td")[3].get_text(),soup.find("table").find_all("tr")[3].find_all("td")[4].get_text().upper(),"GNA "+soup.find("table").find_all("tr")[3].find_all("td")[5].get_text()[:3]
    ...
    def search_name_from_ticket(self,pnr,ticket_number):
        self.search_pnr(pnr)
        response = requests.get(f'http://flyguna.org/SaOnlineReservation/eticket.jsp',cookies=self.cookies,verify=False)
        # print(response.content.decode())
        soup = bs4.BeautifulSoup(response.content,'lxml')
        tables = soup.find_all(attrs={'class':'leble'})
        data = {}
        for i in tables:
            spltted = i.get_text().split()
            print(spltted)
            # print(i.find_all("tr"))
            # print(i.find_all("tr")[17])
            if "PNR" in spltted and i.find_all("tr")[5].find_all("td")[0].get_text().split()[-1] == str(ticket_number):
                passenger_name = i.find_all("tr")[17].find_all("strong")[0].get_text()
