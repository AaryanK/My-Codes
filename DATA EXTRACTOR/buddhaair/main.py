...
class BuddhaAir(AirlineWebsite):
    ...
    def search_pnr_by_ticket_number(self,ticketnumber,**args):
        data = {
        'ddd': '07',
        'mmm': 'APR',
        'yyy': '2022',
        ...
        response = requests.post('http://r2.buddhaair.com/u4OnlineReservation/searchpage.jsp',cookies=self.cookies, data=data, verify=False)
        soup = BeautifulSoup(response.content,'lxml')
        d = soup.find("table").find_all("tr")[3].find_all("td")
        return d[1].get_text(),d[3].get_text(),d[4].get_text().upper(),"U4 "+d[5].get_text()[:3]
    ...
    def date_standarize_issue(self,date):
    ...
    def update_data(self,last_date):
        import datetime
        response = self.session.get('http://r2.buddhaair.com/agentutility/loginagent.jsp', cookies=self.cookies, verify=False)
        MONTHS =["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"]
        x = datetime.datetime.now()
        date_to = x.strftime("%d")+"-"+x.strftime("%b").upper()+"-"+x.strftime("%Y")
        ...
        headers = {
            ...
            # 'Cookie': '_gcl_au=1.1.527595132.1648642309; _ga=GA1.2.1193620680.1648642310; _fbp=fb.1.1648642310052.108547826; _clck=fgpku6|1|f11|0; JSESSIONID=D34572D34EAC27BC5393B094EB34D96A.tomcat2',
            'Origin': 'http://r2.buddhaair.com',
            'Referer': 'http://r2.buddhaair.com/agentutility/salessummary.jsp',
            'Upgrade-Insecure-Requests': '1',
        ...
        data = {
            ...
            'vcurrency': 'ALL',
            'output': 'SCREEN',
            'sales': 'Sales and Refund',
        }

        response = self.session.post('http://r2.buddhaair.com/agentutility/', cookies=self.cookies,data=data, verify=False)
        ...
        for i in sales[1:-1]:
            ...
            js = {
                ...
                'issuedate':self.date_standarize_issue(td[5].get_text().strip()),
