...
class ShreeAir(AirlineWebsite):
    ...
    def search_pnr_by_ticket_number(self,ticketnumber):
        ...
        response = self.session.post('http://shreeair.org/SaOnlineReservation/searchpage.jsp',cookies=self.cookies, data=data, verify=False)
        soup = bs4.BeautifulSoup(response.content,'lxml')
        return soup.find("table").find_all("tr")[3].find_all("td")[1].get_text(),soup.find("table").find_all("tr")[3].find_all("td")[3].get_text(),soup.find("table").find_all("tr")[3].find_all("td")[4].get_text().upper(),"SHA "+soup.find("table").find_all("tr")[3].find_all("td")[5].get_text()[:3]