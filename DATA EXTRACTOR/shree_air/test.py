...
dp = open('shree_air/tickets.json', 'r')
data = json.load(dp)['tickets']
# print(data)
total = len(data)
fp = open('shree_air/names.json', 'r')
det = json.load(fp)
count = 1
error_count = 0
f = open('shree_air/names.json', 'r')
d = json.load(f)
...
for i in data:
    if i['ticket_number'] not in ticket_numbers:
        try:
            ...
            print(a.search_pnr_by_ticket_number(i['ticket_number']))
            ...
            errors = {}
            errors['data'] = i
            fep = open('shree_air/errors.json', 'r')
            delt = json.load(fep)
            delt.append(errors)
            flep = open('shree_air/errors.json', 'w+')
            json.dump(delt,flep)
    count +=1
    print(count)
    # break
fp = open('shree_air/names.json', 'w+')
json.dump(d,fp)
